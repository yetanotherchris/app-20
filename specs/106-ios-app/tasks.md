...
- [ ] T006 Implement document-picker imports, validation, and secure storage for provider and S3 credentials without exposing plaintext to UI state.
- [ ] T007 Implement the OpenRouter streaming client using the secure provider-key service.
- [ ] T008 Select and implement an Expo-compatible signed S3 `SyncRemote`; connect it to the existing sync engine and lifecycle-aware queue.
