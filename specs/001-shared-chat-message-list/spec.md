...
2. **Given** the user is reading earlier content, **When** a new message arrives, **Then** an unread indicator appears.
...
- **FR-003**: The list MUST provide an explicit action to return to the latest message.
- **FR-004**: The list MUST provide a control to load earlier messages when history exists.
- **FR-005**: The list MUST show an unread count for messages below the viewport, not per streaming chunk.
- **FR-006**: Each message MUST keep a stable identity from first render through streaming and edits.
- **FR-007**: A conversation of 1,000 messages MUST render and scroll without perceptible degradation.
...
- **SC-001**: A 1,000-message conversation renders and scrolls without perceptible input lag.
...
- The component is used by a single person on one device; multi-user presence is out of scope.
