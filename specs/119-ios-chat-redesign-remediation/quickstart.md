...
npx vitest run apps/ios/src/chat/conversation.test.ts

# Import rejects duplicates and invalid values without changing the draft.
npx vitest run apps/ios/src/settings/settingsImport.test.ts

# Mirror queue drains and delivers the newest revision after a mid-flight mutation.
npx vitest run apps/ios/src/sync/mirrorQueue.test.ts
