...
```json
{
  ...
  "messages": [
    { "id": "m1", "role": "user", "content": "How do I export a CSV?", "createdAt": "2026-09-10T12:00:00.000Z", "status": "complete" }
  ]
}
```
...
| `onNotification` | `app:notification` | `{ level: 'info' \| 'error'; message: string }` |
...
type WorkspaceInfo = { displayName: string }
type WorkspaceResult = Result<WorkspaceInfo> & { cancelled?: true }
...
|---------|-------|-------------|--------|
| `open-workspace` | Open Workspace Folder... | `CmdOrCtrl+O` | `workspace:choose` |
| `create-workspace` | Create Workspace Folder... | `CmdOrCtrl+Shift+O` | `workspace:create` |
| `import-provider-key` | Import Provider API Key... | `CmdOrCtrl+K` | `secrets:import-provider-key` |
| `import-s3-credentials` | Import S3 Credentials... | `CmdOrCtrl+Shift+K` | `secrets:import-s3` |
| `new-conversation` | New Conversation | `CmdOrCtrl+N` | Sends `menu:command` to renderer |
| `save-document` | Save | `CmdOrCtrl+S` | Sends `menu:command` to renderer |
| `quit` | Quit | `CmdOrCtrl+Q` | Gated quit |

Menu handlers run their operation in main and send `app:notification` with the outcome. Renderer-owned commands (`new-conversation`, `save-document`) send `menu:command`.
