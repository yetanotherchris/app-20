...
**Independent Test**: Send a prompt, let the response finish, and confirm the conversation is on disk with no save action taken.
...
**Independent Test**: Type into the composer, wait briefly, restart the app, and confirm the draft is restored without the message having been sent.

**Acceptance Scenarios**:

1. **Given** the user is typing an unsent message, **When** they stop typing for a short moment, **Then** the draft is persisted without sending and without any save action.
...
**Independent Test**: With a conversation and a draft in progress, close and quit; confirm no dialog appears and everything is persisted.
...
4. **Given** the user typed a draft and quit before the typing timer fired, **When** the app quits, **Then** the pending draft is persisted before exit.
...
**Independent Test**: Inspect the chat screen and menus; confirm no save control or dirty/clean indicator is present.
...
**Independent Test**: Force a save failure and confirm the user is told, the conversation content remains in the session, and the save is retried.
...
2. **Given** an autosave failed, **When** a later autosave succeeds, **Then** the conversation is stored and the failure is cleared.

### Edge Cases

- A response is mid-stream when the app is backgrounded or terminated; the partial content is persisted (also required by spec 105).
- The user quits while a typing-timer save is still pending; the pending change is flushed before exit.
- A save fails at the moment of quitting: the app exits without any prompt; whatever autosave already persisted is on disk, and the failure is not shown that session.
- A new, empty conversation exists; it is valid to persist.
- Two autosaves overlap or arrive close together, including rapid typing; the latest content wins and no write corrupts an earlier one.
- The same conversation is opened and resumed; autosave does not create a duplicate file or duplicate manifest entry.
...
- **FR-002**: The composer draft MUST persist automatically a short moment after the user stops typing, without sending the message and without any save action.
- **FR-003**: The active conversation and its draft MUST persist when the app is closed or quit.
- **FR-004**: A partial assistant response in flight MUST persist when the app is backgrounded or terminated.
...
- **FR-009**: While the app is open, a failed save MUST be reported to the user, MUST retain the conversation content in the session, and MUST be retried; content MUST NOT be silently discarded. At close or quit the app MUST exit without any prompt, after a final attempt to persist.

### Key Entities

- **Conversation**: The persisted chat and its messages; already defined by spec 101.
...
- **SC-001**: After a response completes, 100% of conversations are on disk without any user save action.
- **SC-002**: After the user types and pauses, the draft is on disk before they send or quit; restarting the app restores it.
...
- This supersedes the editor-derived requirements it was seeded with: spec 100 FR-003, US2, and SC-004 (close/quit confirmation) and spec 105 SC-004 plus its assumption that "close and quit confirmation is owned by spec 100". Those are corrected by this feature.
- The constitution's Principle III wording ("a failed save leaves the document dirty") and Principle V's required "dirty/close/quit confirmation" test coverage are editor-shaped and must be amended by a PATCH to describe autosave and atomic writes instead. This feature depends on that amendment.
- The conversation schema, manifest, and atomic file storage are owned by spec 101. The draft stays a field on the conversation, and the typing timer persists the conversation itself. There is exactly one file per conversation plus the manifest; no companion or draft file is added. The trade-off is that each timed save rewrites the whole conversation, which is acceptable at beta scale and mitigated by the typing debounce.
