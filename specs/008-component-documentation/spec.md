...
- Interactive, runnable examples of the component's behavior.
...
- Repeatable automated verification that examples and claims match the released package.

### Out of Scope

- New component capabilities or behavior changes; the documentation describes, it does not extend.
- Marketing or promotional content.
- Examples that require a live backend; examples use deterministic simulated provider responses.
...
3. **Given** the quick-start uses the published package, **When** the steps are completed, **Then** the result uses the component's default presentation and behavior.
...
3. **Given** a documented event or callback, **When** the developer wires it, **Then** the payload shape and timing are described accurately enough to use without reverse engineering.
...
A developer understands how the component is controlled and how to drive it correctly: which values the host owns, how streaming updates arrive, how operation identity prevents stale updates, and how scroll-following behaves.
...
2. **Given** streaming updates, **When** explained, **Then** the guide shows how incremental content reaches the component and how operation identity prevents late updates from changing a newer response.
...
- Every snippet and reference entry is generated from or verified against the published package so drift fails the release; a documented surface that no longer exists in the published package is flagged as broken, not silently dropped.
- An example that fails to build or run fails visibly in the release gate; it never renders as an empty box in production documentation.
- Links within the documentation resolve; broken links are caught by the release gate.
...
- **FR-002**: Documentation MUST provide interactive examples that cover completing a user-to-assistant exchange including streaming, stopping with retained partial content, retrying, regenerating, scrolling, loading earlier messages, and returning to the latest message. Every interactive example MUST run against the released package artifact, not source snapshots or screenshots, and must include the long-conversation and large-Markdown cases defined in the component overview.
- **FR-003**: Documentation MUST contain a complete, enumerable reference covering every exported component, input, output event, callback payload, state, status, content type, renderer contract, and customization slot from the published package. Each entry MUST include the identifier, description, type and signature information, required or optional status, default, callback timing, and at least one minimal usage example. The documented inventory MUST be verified against the published package surface so that omission, removal, or rename failures are caught before release.
- **FR-004**: Documentation MUST include practical recipes, each presented as a minimal end-to-end copyable example with stated assumptions, imports, complete code, expected behavior, and the package version it targets. The recipe set MUST cover theming including high-contrast and enlarged text, custom content rendering with fallback, custom actions or composer controls, long-conversation and scroll behavior, earlier-message loading with a stable reading position, and stopped and error state handling.
...
- **FR-008**: Documentation MUST be structured for low friction: the landing page presents a visible table of contents covering installation, quick start, live examples, concepts, recipes, reference, accessibility, platform notes, and version selection, and search MUST locate every public input, event, type, recipe, and limitation.
- **FR-009**: Documentation MUST be versioned with the component: each release has distinct, addressable pages and a version selector, a changelog, and migration notes for every breaking or behavior-affecting release. Every example, recipe, and reference page MUST declare the package version it targets.
- **FR-010**: Documentation MUST be verified automatically and repeatably against the released package: the verification builds the package, installs that artifact into the examples application, type-checks snippets, builds the documentation, and executes every interactive example scenario, failing when documented exports, snippets, or examples no longer match the package. Releases MUST update the documentation version and changelog together with the package.
...
- **FR-013**: Documentation MUST include a concepts guide, per User Story 6, covering the controlled surface, streaming update flow, operation identity, status transitions, scroll-following rules, and one complete controlled integration.
- **FR-014**: Documentation MUST use concise, task-oriented prose with no marketing or promotional claims, MUST distinguish guarantees from implementation details, and MUST be maintained in the component repository or release-synchronized with it so a change to the component's public surface and its documentation ship together.
...
- **Interactive Example**: A live demonstration of one or more component capabilities using the actual released package.
- **Reference Entry**: A single documented item (config point, event, callback, state, content type, slot) with description, type and signature information, default, and example.
...
- **Verification Gate**: The repeatable automated check that the reference, snippets, and examples match the released package.
...
- **SC-003**: A complete mapping is produced from every input, event, callback payload, state, status, content type, and customization slot defined in the published package surface to a reference entry; the requirement passes only when the mapping is exhaustive and every entry has a description, default, and example.
...
- **SC-008**: The automated verification gate passes before release: reference inventory, every snippet, and every interactive example scenario match the released package, and a deliberate mismatch fails the gate.
- **SC-009**: Each released version has distinct, addressable documentation pages and a published changelog; all examples, recipes, and reference pages declare the package version they target.

## Assumptions

- The documentation deliverable is the versioned resource integrators use to determine the documented public surface of the published component package; its quality bar is defined by the characteristics of the cited single-component examples.
- "Live interactive examples" means executable demonstrations of the component itself using the released package artifact and deterministic simulated provider responses; examples do not require a live network backend.
- Documentation is published as a public static website with per-version addressing and in-browser example execution; the specific hosting platform is chosen during planning.
