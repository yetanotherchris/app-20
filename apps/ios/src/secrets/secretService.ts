...
export type SecretErrorCode =
  'chooser-cancelled' | 'invalid-secret' | 'multiple-secrets' | 'unknown'
...
function validateS3(raw: string): string | null {
  ...
  if (
    bucket !== undefined &&
    (typeof bucket !== 'string' || !S3_BUCKET_PATTERN.test(bucket.trim()))
  )
    return null
  if (
    region !== undefined &&
    (typeof region !== 'string' || !S3_REGION_PATTERN.test(region.trim()))
  )
    return null
  if (endpoint !== undefined && (typeof endpoint !== 'string' || !isHttpUrl(endpoint.trim())))
    return null
  ...
  async function importSecret(kind: SecretKind): Promise<SecretResult> {
    ...
    try {
      ...
      await SecureStore.setItemAsync(
        kind === 'provider-key' ? PROVIDER_KEY_STORAGE_KEY : S3_STORAGE_KEY,
        normalized,
      )
      return { ok: true }
    } catch {
