...
describe('parseSettingsImport', () => {
  ...
  it('rejects a duplicate JSON property name (FR-014)', () => {
    expect(
      parseSettingsImport(
        'settings.json',
        '{"apiKey":"one","apiKey":"two","s3":{"bucket":"weekend-chat-beta"}}',
      ),
    ).toEqual({
      ok: false,
  ...
  it('rejects duplicate nested S3 property names (FR-014)', () => {
    const result = parseSettingsImport(
      'settings.json',
      '{"s3":{"bucket":"weekend-chat-beta","bucket":"other-bucket"}}',
    )
  ...
  it('rejects a non-HTTPS endpoint (FR-013)', () => {
    expect(
      parseSettingsImport('settings.json', '{"s3":{"endpoint":"http://example.com"}}'),
    ).toEqual({
      ok: false,
