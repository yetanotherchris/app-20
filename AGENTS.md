...
When writing or editing documents in this repo, use an engineer-to-engineer voice. You are recording information, not writing prose: state facts, constraints, and decisions directly and factually, with no editorializing, persuasive framing, or rhetorical emphasis.
...
- Each implementation-PR review comment MUST be a substantive, concern-specific report. A bare approval, a list of severities without evidence, or a one-paragraph summary is not a review.
  Every comment MUST include:
  - A heading that names the review concern and a one-sentence scope stating the diff range and boundaries reviewed.
  - At least two verified behaviors or constraints, each with repository file and line references and a short explanation of why the evidence satisfies the concern.
  - A `### Findings` section. Each finding MUST state severity, file and line reference, concrete failure path or maintenance consequence, and a proposed fix. Findings are ordered critical, major, minor, then nit.
  - When there are no findings, an explicit `No findings` statement followed by residual risks, untested cases, or constraints that limit the review. Do not invent findings to fill the section.
  - Test evidence relevant to the concern: commands run and result, or a clear statement that no command was run and why.
  - The required model attribution as the final line.
- Review agents MUST inspect the full PR diff against `main`, the relevant specification and plan, and the code or tests that implement the reviewed behavior. They must not infer behavior from the PR description alone.
- Before posting a review, the coordinating agent MUST inspect the proposed comment for the required sections, evidence, actionable findings, and attribution. Request a revised review from the agent instead of posting an incomplete report.
- Review dispositions must cite the original comment ID, state whether each finding was fixed, deferred, or needs no action, and include the fix commit for critical and major findings.
