...
export type HamburgerItem =
  ...
  | {
      kind: 'action'
      label: string
      action: 'clear-recent' | 'toggle-devtools' | 'settings' | 'quit'
    }
```
...
The mind prefers a simple whole. Format so the reader first sees _one_ operation (a chain, a component, a type, a handler), not a scatter of tokens.
...
const sourceView = document.view === 'source' && <SourceView value={document.content} /* … */ />
```
...
- Preload bridges stay a single object literal of named methods — that object _is_ the API surface; keep it one dense, uniform map.

**CSS**

- A selector block is one form: properties inside, no blank lines mid-rule. Blank lines go _between_ components, not between `display` and `align-items`.
...
- `ok(receipt)` / `return err(...)` stay immediately after the operation they report; do not insert logging or unrelated assignments between success path and return when the return _is_ the result.
...
- JSX attributes: once a tag is multi-line, _all_ attributes are multi-line (continuation of “one prop per line”). Do not leave the first two props on the opening line and hang the rest below.
...
| Vertical chains and attribute lists when the construct _is_ the sentence           | Zigzag: half a chain inline, half broken, props half-and-half |                                                                           |
...
- Saves are **atomic** (temp file in the same directory, then rename). A failed save leaves the prior content intact and the in-session content available; it is reported and retried, never silently dropped (constitution III).
...
- **Non-negotiable coverage** (constitution V): path containment (adversarial cases), atomic write and save-failure, autosave and failed-save reporting, close and quit with no prompt, IPC contract shape, markdown round-trip where the editor might mangle content.
...
- Coverage and suite health: constitution-required areas (path, save, autosave, IPC shape) are **gates**: those tests must exist and pass. Broader coverage percentages are a **signal**; use CI floors if the team wants them, but do not treat a one-point coverage dip as more important than the non-negotiable scenarios.
...
- [ ] Saves atomic; failed save reported and retried; no silent discard of content
...
_Coding standards are living guidance. Amend this file when the team learns a better default; do not fork silent local conventions that contradict it._
